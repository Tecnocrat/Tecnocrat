...
1. The `Tecnocrat/AIOS` GitHub Actions workflow runs daily at 03:17 UTC and can
...
4. If the catalog changes, the workflow commits the new observation.
5. `Tecnocrat/aios-api` independently refreshes a public-only projection at the
   same daily cadence. The API serves it at `/api/repositories` and
   `/data/tecnocrat-repository-catalog.json`.
...
The API catalog contains only public repositories. Private repositories are
filtered before writing that public snapshot, so the public face can describe
the visible portfolio without exposing private repository names or metadata.
